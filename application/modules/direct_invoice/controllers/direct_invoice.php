...
class Direct_invoice extends MX_Controller{
   function __construct() {
    ...
    function index(){     
        // EAN-8, EAN-13, UPC-A
//[12:52:49 PM | Edited 12:52:56 PM] Tommy King: EAN-8 has 8 character, EAN-13 has 13, UPC-A has 12
   ...
   function get_item_details(){
    ...
    /* get purchase order 
     function start     */
    function  view_direct_invoice($guid){
        if($this->session->userdata['direct_invoice_per']['view']==1){
        $this->load->model('purchase');
        $data=  $this->purchase->get_direct_invoice($guid);
        echo json_encode($data);
        }
    ...
    /* get purchase order and invoice settings
     function start     */
    function  get_invoice_settings_and_direct_invoice($guid){
        if($this->session->userdata['direct_invoice_per']['print_invoice']==1){
        $this->load->model('purchase');
        $data[0]=  $this->purchase->direct_invoice_invoice($guid); // get purchas eorder details
         // read setting config file
        if($this->session->flashdata('direct_invoice_invoice')==""){
            $this->config->load("settings");
            $value=array();
            $value=$this->config->item('invoice');
            $this->session->set_flashdata('direct_invoice_invoice',$value); 
            $data[1]=$value; 
        }else{        
            $data[1]=$this->session->flashdata('direct_invoice_invoice'); // get invoice array
        }
        echo json_encode($data);
        }
    }
    ...
    function save_invoice_settings(){
                ...
                $posnic_direct_invoice_subtotal=$this->input->post('posnic_direct_invoice_subtotal')==1?1:0;
                $data=$data.'"posnic_direct_invoice_subtotal"=>'.$posnic_direct_invoice_subtotal.','."\n";
                ...
                write_file('application/modules/direct_invoice/config/settings.php', $data);
                $this->config->load("settings");
                $this->session->set_flashdata('direct_invoice_invoice', $this->config->item('invoice'));
                echo 'TRUE';
    }
