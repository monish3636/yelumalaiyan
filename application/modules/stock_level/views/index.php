...
<script type="text/javascript">
    ...
    function update_stock(){
        <?php if($this->session->userdata['stock_level_per']['edit']==1){ ?>
            if($('#parsley_reg').valid()){
                var inputs = $('#parsley_reg').serialize();
                $.ajax ({
                    url: "<?php echo base_url('index.php/stock_level/update')?>",
                    data: inputs,
                    type:'POST',
                    complete: function(response) {
                        if(response['responseText']=='TRUE'){
                              $.bootstrapGrowl('<?php echo $this->lang->line('stock_level').' '.$this->lang->line('updated');?>', { type: "success" });                                                                                  
                               $("#dt_table_tools").dataTable().fnDraw();
                               $("#parsley_reg").trigger('reset');
                               posnic_stock_level_lists();
                              ...
                              $.bootstrapGrowl('<?php echo $this->lang->line('You Have NO Permission To Add')." ".$this->lang->line('stock_level');?>', { type: "error" });                           
                        }
                    }
                });
            }else{
