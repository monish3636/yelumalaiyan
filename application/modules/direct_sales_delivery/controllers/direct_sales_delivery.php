...
function save(){      
     if($this->session->userdata['direct_sales_delivery_per']['add']==1){
        ...
        $this->form_validation->set_rules('new_item_tax[]', $this->lang->line('new_item_tax_value'), 'numeric');                      
            ...
            if ( $this->form_validation->run() !== false ) {    
              ...
              $guid=   $this->posnic->posnic_add_record($value,'direct_sales_delivery');
                ...
                for($i=0;$i<count($item);$i++){
                    ...
                    $this->sales->add_direct_sales_delivery($guid,$item[$i],$quty[$i],$price[$i],$stock[$i],$item_discount[$i],$item_tax[$i],$item_tax2[$i],$i);
    ...
    function update(){
      ...
      if($this->session->userdata['direct_sales_delivery_per']['edit']==1){
        ...
        $this->form_validation->set_rules('new_item_tax_value[]', $this->lang->line('new_item_tax_value'), 'numeric');                      
        ...
        $this->form_validation->set_rules('items_stock[]', $this->lang->line('items_stock')); 
            ...
            if ( $this->form_validation->run() !== false ) {    
                ...
                for($i=0;$i<count($sq);$i++){
                    $this->sales->update_direct_sales_delivery($sq[$i],$quty[$i],$price[$i],$item_tax[$i],$item_tax2[$i]);
                ...
                if(count($new_stock)>0){
                    for($i=0;$i<count($new_stock);$i++){
                        if($new_item[$i]!="" || $new_item[$i]!=0){
                            $this->sales->add_direct_sales_delivery($guid,$new_item[$i],$new_quty[$i],$new_price[$i],$new_stock[$i],$new_item_discount[$i],$new_item_tax[$i],$new_item_tax2[$i],$i);
                        }
                    }
                }
...
// function end
    ...
    function  view_direct_sales_delivery($guid){
...
function search_items(){
    ...
    function get_invoice_settings_and_direct_sales_delivery($guid){
        if($this->session->userdata['direct_sales_delivery_per']['print_invoice']==1){
            $this->load->model('sales');
            $data[0]=  $this->sales->direct_sales_delivery_invoice($guid); // get purchas eorder details
             // read setting config file
            if($this->session->flashdata('direct_sales_delivery_invoice')==""){
                $this->config->load("settings");
                $value=array();
                $value=$this->config->item('invoice');
                $this->session->set_flashdata('direct_sales_delivery_invoice',$value); 
                $data[1]=$value; 
            }else{        
                $data[1]=$this->session->flashdata('direct_sales_delivery_invoice'); // get invoice array
            }
            echo json_encode($data);
        }
    }
 /* get purchase order invoice getting details*/
    ...
    function save_invoice_settings(){
                ...
                $posnic_direct_sales_delivery_subtotal=$this->input->post('posnic_direct_sales_delivery_subtotal')==1?1:0;
                $data=$data.'"posnic_direct_sales_delivery_subtotal"=>'.$posnic_direct_sales_delivery_subtotal.','."\n";
                ...
                write_file('application/modules/direct_sales_delivery/config/settings.php', $data);
                $this->config->load("settings");
                $this->session->set_flashdata('direct_sales_delivery_invoice', $this->config->item('invoice'));
                echo 'TRUE';
    }
}
?>
