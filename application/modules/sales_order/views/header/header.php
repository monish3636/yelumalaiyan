...
<script type="text/javascript" charset="utf-8">
    ...
    function sales_order_view(guid){
        $('#sales_order_select').hide();
        $('#customer_select').show();
        <?php
        if($this->session->userdata['sales_order_per']['edit']==1){ ?>                                
            $('#deleted').remove();
    ...
    function sales_order_invoice(guid){
        ...
        if($this->session->userdata['sales_order_per']['print_invoice']==1){ ?>
            ...
            $('#sales_order_lists').removeAttr("disabled");
            $('#invoice_div').show();
            $('#sales_order_guid').val(guid);
            $.ajax({                                      
                url: "<?php echo base_url() ?>index.php/sales_order/get_invoice_settings_and_sales_order/"+guid,                      
                data: "", 
                dataType: 'json',               
                success: function(data)        
                { 
                     ...
                     $('#invoice_posnic_table').append('<thead/><tbody/><tfoot/>');
                        ...
                        var sales_order_discount=0;                        
                        var sales_order_frieght=0;                        
                        var sales_order_round_off_amount=0;                        
                        var sales_order_sub_total=0;                        
                        var sales_order_grand=0;                        
                        for(var i=0;i<data[0].length;i++){
                            ...
                            sales_order_sub_total=parseFloat(sales_order_sub_total)+parseFloat(total);
                        ...
                        if(data[1]['posnic_sales_order_subtotal']==1){
                            var num = parseFloat(sales_order_sub_total);
                            sales_order_sub_total=num.toFixed(point);
                            ...
                            $('#posnic_table_tfoot1').append('<td class="text-right">'+sales_order_sub_total+' </td>');                            
                        }
    ...
    function invoice_settings(){
        ...
        if($this->session->userdata['sales_order_per']['invoice_setting']==1){ ?>
            ...
            $.ajax({                                      
                url: "<?php echo base_url('sales_order/get_invoice_settings') ?>",                      
                data: "", 
                dataType: 'json',               
                success: function(data) {
    ...
    function save_invoice_settings(){
        ...
        if($this->session->userdata['sales_order_per']['invoice_setting']==1){ ?>
              $('#loading').modal('show');
                    ...
                    $.ajax ({
                        url: "<?php echo base_url('index.php/sales_order/save_invoice_settings')?>",
                        data: inputs,
                        type:'POST',
                        complete: function(response) {
                            if(response['responseText']=='TRUE'){
                                ...
                                sales_order_invoice($('#sales_order_guid').val());
                                $('#loading').hide();
                            }else{
                                $.bootstrapGrowl('<?php echo $this->lang->line('you_have_no_permission_to_update')." ".$this->lang->line('invoice_settings'); ?>', { type: "error" });          
                            }
