...
class Payment extends CI_Model{
   ...
   /* function end*/
    ...
    function save_payment($payment,$amount,$date,$memo,$code,$invoice_id){
        ...
        $data=array('invoice_id'=>$invoice_id,'code'=>$code,'type'=>'credit','payable_id'=>$payment,'customer_id'=>$supplier,'memo'=>$memo,'amount'=>$amount,'payment_date'=>$date,'added_date'=>strtotime(date("Y/m/d H:i")),'branch_id'=>  $this->session->userdata['branch_id'],'added_by'=>  $this->session->userdata['guid']);
    ...
    function sales_return_payment($amount,$date,$memo,$code,$customer,$invoice_id,$return_id){
        ...
        $data=array('invoice_id'=>$invoice_id,'return_id'=>$return_id,'code'=>$code,'type'=>'credit','customer_id'=>$customer,'memo'=>$memo,'amount'=>$amount,'payment_date'=>$date,'added_date'=>strtotime(date("Y/m/d H:i")),'branch_id'=>  $this->session->userdata['branch_id'],'added_by'=>  $this->session->userdata['guid']);
        $this->db->insert('payment',$data);
         $id=  $this->db->insert_id();
        $this->db->where('id',$id);
