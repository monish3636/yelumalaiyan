...
$("#parsley_reg #first_name").select2('data', {id:'',text: 'Search Supplier'});
    ...
    $('#add_new_order').show('slow');
      ...
      $('#posnic_add_opening_stock').attr("disabled", "disabled");
...
function posnic_opening_stock_lists(){
      ...
      $('#posnic_add_opening_stock').removeAttr("disabled");
      $('#opening_stock_lists').attr("disabled",'disabled');
}
function clear_add_opening_stock(){
      $("#parsley_reg").trigger('reset');
      refresh_items_table();
}
function clear_update_opening_stock(){
      $("#parsley_reg").trigger('reset');
      refresh_items_table();
      edit_function($('#opening_stock_guid').val());
}
...
function add_new_price(e){          
 ...
 function add_new_date(e){
       if($('#parsley_reg #item_id').val()!=""){
        ...
        var unicode=e.charCode? e.charCode : e.keyCode
    if($('#parsley_reg #quantity').val()!="" && $('#parsley_reg #cost').val() && $('#parsley_reg #price').val()){
                  if (unicode!=13 && unicode!=9){
...
function copy_items(){
 ...
 if( $('#parsley_reg #supplier_id').val()!="" && $('#parsley_reg #item_id').val()!="" &&  $('#parsley_reg #cost').val()!="" && $('#parsley_reg #price').val()!=""  && $('#parsley_reg #quantity').val()!=""){
...
if(document.getElementById('new_item_row_id_'+$('#parsley_reg #item_id').val())){
 ...
 var old_total= $('#selected_item_table #new_item_row_id_'+$('#parsley_reg #item_id').val()+' #items_total').val();
  ...
  $('#selected_item_table #new_item_row_id_'+$('#parsley_reg #item_id').val()+' td:nth-child(8)').html(discount);
  $('#selected_item_table #new_item_row_id_'+$('#parsley_reg #item_id').val()+' td:nth-child(9)').html($('#parsley_reg #first_name').select2('data').text);
  ...
  $('#newly_added #new_item_supplier'+items_id).val($('#parsley_reg #first_name').select2('data').id);
  ...
  $('#selected_item_table #new_item_row_id_'+$('#parsley_reg #item_id').val()+' #items_supplier').val($('#parsley_reg #first_name').select2('data').id);
...
<input type="hidden" name="new_item_supplier[]" value="'+$('#parsley_reg #first_name').select2('data').id+'" id="new_item_supplier_'+items_id+'">\n\
...
<input type="hidden" name="new_item_total[]"  value="'+parseFloat(quty)*parseFloat(cost)+'" id="new_item_total_'+items_id+'">\n\
   ...
   var addId = $('#selected_item_table').dataTable().fnAddData( [
      ...
      tax+' : '+tax_type+'('+type+')',
      discount,
      $('#parsley_reg #first_name').select2('data').text,
      total,
...
<input type="hidden" name="items_supplier[]" value="'+$('#parsley_reg #first_name').select2('data').id+'" id="items_supplier">\n\
<input type="hidden" name="items_supplier_name[]" value="'+$('#parsley_reg #first_name').select2('data').text+'" id="items_supplier_name"> \n\
...
function edit_order_item(guid){
    ...
    $("#parsley_reg #first_name").select2('data', {id:$('#selected_item_table #new_item_row_id_'+guid+' #items_supplier').val(),text: $('#selected_item_table #new_item_row_id_'+guid+' #items_supplier_name').val()});
...
<section id="add_new_order" class="container clearfix main_section">
     <?php   $form =array('id'=>'parsley_reg',
       ...
       echo form_open_multipart('opening_stock/upadate_pos_opening_stock_details/',$form);?>
    ...
    <div id="main_content" style="padding: 0 14px !important;">
        ...
        <input type="hidden" name="opening_stock_guid" id="opening_stock_guid" >
