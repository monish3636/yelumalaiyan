...
function direct_invoice_approve(guid){
    ...
    function direct_invoice_view(guid){
        <?php
        if($this->session->userdata['direct_invoice_per']['edit']==1){ ?>
            $('#deleted').remove();
            $('#parent_items').append('<div id="deleted"></div>');
    ...
    function direct_invoice_invoice(guid){
        <?php
        if($this->session->userdata['direct_invoice_per']['print_invoice']==1){ ?>
            $('#loading').modal('show');
            $("#user_list").hide();
            $('#add_new_order').hide();
            ...
            $('#direct_invoice_lists').removeAttr("disabled");
            $('#invoice_div').show();
            $('#direct_invoice_guid').val(guid);
            $.ajax({                                      
                url: "<?php echo base_url() ?>index.php/direct_invoice/get_invoice_settings_and_direct_invoice/"+guid,                      
                data: "", 
                dataType: 'json',               
                success: function(data)        
                { 
                        if(data[1]['posnic_order_id']==1){
                            ...
                            $('#invoice_posnic_id').html(data[0][0]['invoice_no'])
                        }else{
                        ...
                        if(data[1]['posnic_date']==1){
                            $('#invoice_posnic_date').show();
                              $('#invoice_posnic_date').html('<?php echo $this->lang->line('date') ?> : <span class="text-muted">'+data[0][0]['invoice_date']+'</span>');
                        }else{
                     ...
                     $('#invoice_posnic_table').append('<thead/><tbody/><tfoot/>');
                        ...
                        var direct_invoice_discount=0;                        
                        var direct_invoice_frieght=0;                        
                        var direct_invoice_round_off_amount=0;                        
                        var direct_invoice_sub_total=0;                        
                        var direct_invoice_grand=0;                        
                        for(var i=0;i<data[0].length;i++){
                            ...
                            var num = parseFloat(subtotal);
                            subtotal=num.toFixed(point);
                            direct_invoice_sub_total=parseFloat(direct_invoice_sub_total)+parseFloat(total);
                        ...
                        if(data[1]['posnic_direct_invoice_subtotal']==1){
                            var num = parseFloat(direct_invoice_sub_total);
                            direct_invoice_sub_total=num.toFixed(point);
                            ...
                            $('#posnic_table_tfoot1').append('<td class="text-right">'+direct_invoice_sub_total+' </td>');                            
                        }
  ...
  function invoice_disable(){
    ...
    function invoice_settings(){
        <?php
        if($this->session->userdata['direct_invoice_per']['invoice_setting']==1){ ?>
            $('#loading').modal('show');
            $('#invoice_div').hide();
            $('#invoice_settings').show('slow');
            $.ajax({                                      
                url: "<?php echo base_url('direct_invoice/get_invoice_settings') ?>",                      
                data: "", 
                dataType: 'json',               
                success: function(data) {
                    ...
                    data['posnic_direct_invoice_subtotal']==1?$('#posnic_direct_invoice_subtotal').attr('checked','checked'):$('#posnic_direct_invoice_subtotal').removeAttr('checked');
    ...
    function save_invoice_settings(){
        ...
        if($this->session->userdata['direct_invoice_per']['invoice_setting']==1){ ?>
              $('#loading').modal('show');
                    ...
                    $.ajax ({
                        url: "<?php echo base_url('index.php/direct_invoice/save_invoice_settings')?>",
                        data: inputs,
                        type:'POST',
                        complete: function(response) {
                            if(response['responseText']=='TRUE'){
                                ...
                                direct_invoice_invoice($('#direct_invoice_guid').val());
                                $('#loading').hide();
                            }else{
                                $.bootstrapGrowl('<?php echo $this->lang->line('you_have_no_permission_to_update')." ".$this->lang->line('invoice_settings'); ?>', { type: "error" });          
                            }
