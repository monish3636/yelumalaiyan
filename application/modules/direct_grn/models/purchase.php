...
class Purchase extends CI_Model{
    ...
    function get_direct_grn($guid){
        $this->db->select('items.tax_inclusive2,items.tax2_type,items.tax2_value,items.tax_Inclusive ,tax_types.type as tax_type_name,taxes.value as tax_value,taxes.type as tax_type,suppliers_x_items.quty as item_limit,suppliers.guid as s_guid,suppliers.first_name as s_name,suppliers.company_name as c_name,suppliers.address1 as address,direct_grn.*,purchase_items.discount_per as dis_per ,purchase_items.discount_per2 as dis_per2 ,purchase_items.discount_amount as item_dis_amt ,purchase_items.discount_amount2 as item_dis_amt2 ,purchase_items.tax2 as order_tax2 ,purchase_items.tax as dis_amt ,purchase_items.tax as order_tax,purchase_items.item ,purchase_items.quty ,purchase_items.free ,purchase_items.cost ,purchase_items.sell ,purchase_items.mrp,purchase_items.guid as o_i_guid ,purchase_items.amount ,items.guid as i_guid,items.name as items_name,items.code as i_code')->from('direct_grn')->where('direct_grn.guid',$guid);
    ...
    function direct_grn_invoice($guid){
        $this->db->select('branches.code as branch_code,branches.store_name as branch_name,branches.address as branch_address,branches.city as branch_city,branches.state as branch_state,branches.zip as branch_zip ,branches.country as branch_country,branches.phone as branch_phone,branches.email as branch_mail,suppliers.email as supplier_email,suppliers.phone as supplier_phone,suppliers.city as supplier_city,suppliers.state as supplier_state,suppliers.zip as supplier_zip,suppliers.country as supplier_country,items.tax_inclusive2,items.tax2_type,items.tax2_value,items.tax_Inclusive ,tax_types.type as tax_type_name,taxes.value as tax_value,taxes.type as tax_type,suppliers_x_items.quty as item_limit,suppliers.guid as s_guid,suppliers.first_name as s_name,suppliers.company_name as c_name,suppliers.address1 as address,direct_grn.*,purchase_items.discount_per as dis_per ,purchase_items.discount_per2 as dis_per2 ,purchase_items.discount_amount as item_dis_amt ,purchase_items.discount_amount2 as item_dis_amt2 ,purchase_items.tax2 as order_tax2 ,purchase_items.tax as dis_amt ,purchase_items.tax as order_tax,purchase_items.item ,purchase_items.quty ,purchase_items.free ,purchase_items.cost ,purchase_items.sell ,purchase_items.mrp,purchase_items.guid as o_i_guid ,purchase_items.amount ,items.guid as i_guid,items.name as items_name,items.code as i_code')->from('direct_grn')->where('direct_grn.guid',$guid);
        $this->db->join('purchase_items', 'purchase_items.order_id = direct_grn.guid ','left');
    ...
    function direct_grn_stock($guid,$Bid){
        ...
        foreach ($grn->result() as $grn_row){
            ...
            $this->db->update('stock',array('date'=>strtotime(date('d-m-Y')),'quty'=>$grn_row->quty+$stock_quty,'price'=>$price));
             ...
             $this->db->insert('stock',array('date'=>strtotime(date('d-m-Y')),'item'=>$grn_row->item,'quty'=>$grn_row->quty,'price'=>$price,'branch_id'=>$Bid));
            $id=  $this->db->insert_id();
            $this->db->where('id',$id);
