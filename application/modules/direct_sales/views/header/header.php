...
function direct_sales_approve(guid){
    ...
    function direct_sales_view(guid){
        $('#sales_items_list').show();
        $('#sales_buttons').show();
        $('#address_div').show();
        $('#sales_bill_buttons').hide();
        ...
        if($this->session->userdata['direct_sales_per']['view']==1){ ?>
            ...
            $('#loading').modal('show');
            $.ajax({                                      
                url: "<?php echo base_url() ?>index.php/direct_sales/view_direct_sales/"+guid,                      
                data: "", 
                dataType: 'json',               
                success: function(data)        
                { 
    ...
    function direct_sales_invoice(guid){
        <?php
        if($this->session->userdata['direct_sales_per']['print_invoice']==1){ ?>
            ...
            $('#direct_sales_lists').removeAttr("disabled");
            $('#invoice_div').show();
            $('#direct_sales_guid').val(guid);
            $.ajax({                                      
                url: "<?php echo base_url() ?>index.php/direct_sales/get_invoice_settings_and_direct_sales/"+guid,                      
                data: "", 
                dataType: 'json',               
                success: function(data)        
                { 
                     ...
                     $('#invoice_posnic_table').append('<thead/><tbody/><tfoot/>');
                        ...
                        var direct_sales_discount=0;                        
                        var direct_sales_frieght=0;                        
                        var direct_sales_round_off_amount=0;                        
                        var direct_sales_sub_total=0;                        
                        var direct_sales_grand=0;                        
                        for(var i=0;i<data[0].length;i++){
                            ...
                            direct_sales_sub_total=parseFloat(direct_sales_sub_total)+parseFloat(total);
                        ...
                        if(data[1]['posnic_direct_sales_subtotal']==1){
                            var num = parseFloat(direct_sales_sub_total);
                            direct_sales_sub_total=num.toFixed(point);
                            ...
                            $('#posnic_table_tfoot1').append('<td class="text-right">'+direct_sales_sub_total+' </td>');                            
                        }
    ...
    function invoice_settings(){
        ...
        if($this->session->userdata['direct_sales_per']['invoice_setting']==1){ ?>
            ...
            $.ajax({                                      
                url: "<?php echo base_url('direct_sales/get_invoice_settings') ?>",                      
                data: "", 
                dataType: 'json',               
                success: function(data) {
                    ...
                    data['posnic_direct_sales_subtotal']==1?$('#posnic_direct_sales_subtotal').attr('checked','checked'):$('#posnic_direct_sales_subtotal').removeAttr('checked');
    ...
    function save_invoice_settings(){
        ...
        if($this->session->userdata['direct_sales_per']['invoice_setting']==1){ ?>
              $('#loading').modal('show');
                    ...
                    $.ajax ({
                        url: "<?php echo base_url('index.php/direct_sales/save_invoice_settings')?>",
                        data: inputs,
                        type:'POST',
                        complete: function(response) {
                            if(response['responseText']=='TRUE'){
                                ...
                                direct_sales_invoice($('#direct_sales_guid').val());
                                $('#loading').hide();
                            }else{
                                $.bootstrapGrowl('<?php echo $this->lang->line('you_have_no_permission_to_update')." ".$this->lang->line('invoice_settings'); ?>', { type: "error" });          
                            }
