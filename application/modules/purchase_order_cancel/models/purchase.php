...
class Purchase extends CI_Model{
    ...
    function search_items($search,$bid,$guid,$limit){
        $this->db->select('purchase_items.*,items_setting.purchase,items.tax_Inclusive ,tax_types.type as tax_type_name,taxes.value as tax_value,taxes.type as tax_type,brands.name as b_name,items_department.department_name as d_name,items_category.category_name as c_name,items.name,items.guid as i_guid,items.code,items.image,items.tax_Inclusive,items.tax_id,')->from('purchase_items')->where('purchase_items.order_id',$guid);
