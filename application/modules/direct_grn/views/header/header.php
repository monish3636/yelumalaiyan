...
function direct_grn_approve(guid){
    ...
    function direct_grn_view(guid){
        ...
        if($this->session->userdata['direct_grn_per']['view']==1){ ?>
            ...
            $('#loading').modal('show');
            $.ajax({                                      
                url: "<?php echo base_url() ?>index.php/direct_grn/direct_grn_view/"+guid,                      
                data: "", 
                dataType: 'json',               
                success: function(data)        
                { 
    ...
    function direct_grn_invoice(guid){
        ...
        if($this->session->userdata['direct_grn_per']['print_invoice']==1){ ?>
            ...
            $('#direct_grn_lists').removeAttr("disabled");
            $('#invoice_div').show();
            $('#direct_grn_guid').val(guid);
            $.ajax({                                      
                url: "<?php echo base_url() ?>index.php/direct_grn/get_invoice_settings_and_direct_grn/"+guid,                      
                data: "", 
                dataType: 'json',               
                success: function(data)        
                { 
                        if(data[1]['posnic_order_id']==1){
                            ...
                            $('#invoice_posnic_id').html(data[0][0]['grn_no'])
                        }else{
                     ...
                     $('#invoice_posnic_table').append('<thead/><tbody/><tfoot/>');
                        ...
                        var direct_grn_discount=0;                        
                        var direct_grn_frieght=0;                        
                        var direct_grn_round_off_amount=0;                        
                        var direct_grn_sub_total=0;                        
                        var direct_grn_grand=0;                        
                        for(var i=0;i<data[0].length;i++){
                            ...
                            var num = parseFloat(subtotal);
                            subtotal=num.toFixed(point);
                            direct_grn_sub_total=parseFloat(direct_grn_sub_total)+parseFloat(total);
                        ...
                        if(data[1]['posnic_direct_grn_subtotal']==1){
                            var num = parseFloat(direct_grn_sub_total);
                            direct_grn_sub_total=num.toFixed(point);
                            ...
                            $('#posnic_table_tfoot1').append('<td class="text-right">'+direct_grn_sub_total+' </td>');                            
                        }
    ...
    function invoice_settings(){
        ...
        if($this->session->userdata['direct_grn_per']['invoice_setting']==1){ ?>
            ...
            $.ajax({                                      
                url: "<?php echo base_url('direct_grn/get_invoice_settings') ?>",                      
                data: "", 
                dataType: 'json',               
                success: function(data) {
                    ...
                    data['posnic_direct_grn_subtotal']==1?$('#posnic_direct_grn_subtotal').attr('checked','checked'):$('#posnic_direct_grn_subtotal').removeAttr('checked');
    ...
    function save_invoice_settings(){
        ...
        if($this->session->userdata['direct_grn_per']['invoice_setting']==1){ ?>
              $('#loading').modal('show');
                    ...
                    $.ajax ({
                        url: "<?php echo base_url('index.php/direct_grn/save_invoice_settings')?>",
                        data: inputs,
                        type:'POST',
                        complete: function(response) {
                            if(response['responseText']=='TRUE'){
                                ...
                                direct_grn_invoice($('#direct_grn_guid').val());
                                $('#loading').hide();
                            }else{
                                $.bootstrapGrowl('<?php echo $this->lang->line('you_have_no_permission_to_update')." ".$this->lang->line('invoice_settings'); ?>', { type: "error" });          
                            }
