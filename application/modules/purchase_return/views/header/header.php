...
function purchase_return_approve(guid){
    ...
    function purchase_return_view(guid){
        <?php
        if($this->session->userdata['purchase_return_per']['view']==1){ ?>
            ...
            $('#loading').modal('show');
            $.ajax({                                      
                url: "<?php echo base_url() ?>index.php/purchase_return/view_purchase_return/"+guid,                      
                data: "", 
                dataType: 'json',               
                success: function(data)        
                { 
    ...
    function purchase_return_invoice(guid){
        ...
        if($this->session->userdata['purchase_return_per']['print_invoice']==1){ ?>
            ...
            $('#purchase_return_lists').removeAttr("disabled");
            $('#invoice_div').show();
            $('#purchase_return_guid').val(guid);
            $.ajax({                                      
                url: "<?php echo base_url() ?>index.php/purchase_return/get_invoice_settings_and_purchase_return/"+guid,                      
                data: "", 
                dataType: 'json',               
                success: function(data)        
                { 
                        if(data[1]['posnic_purchase_retun_id']==1){
                            $('#invoice_posnic_purchase_retun_id').show();
                            $('#invoice_posnic_purchase_retun_id').html(data[0][0]['code'])
                        }else{
                            $('#invoice_posnic_purchase_retun_id').hide();
                        }
                        if(data[1]['posnic_purchase_retun_no']==1){
                            $('#invoice_posnic_purchase_retun_no').show();
                            $('#invoice_posnic_purchase_retun_no').html('<?php echo $this->lang->line('purchase_return')." ".$this->lang->line('no') ?> <span class="text-muted "> #'+data[0][0]['invoice_id']+'</span>')
                        }else{
                            $('#invoice_posnic_purchase_retun_no').hide();
                        }
                        if(data[1]['posnic_purchase_retun_date']==1){
                            $('#invoice_posnic_purchase_retun_date').show();
                              $('#invoice_posnic_purchase_retun_date').html('<?php echo $this->lang->line('purchase_return')." ".$this->lang->line('date') ?> : <span class="text-muted">'+data[0][0]['date']+'</span>');
                        }else{
                            $('#invoice_posnic_purchase_retun_date').hide();
                        }
                        if(data[1]['posnic_order_id']==1){
                            $('#invoice_posnic_id').show();
                            $('#invoice_posnic_id').html('<?php echo $this->lang->line('purchase_invoice') ?> <span class="text-muted "> #'+data[0][0]['invoice']+'</span>')
                        }else{
                            $('#invoice_posnic_id').hide();
                        }
                        if(data[1]['posnic_number']==1){
                            $('#invoice_posnic_number').show();
                            $('#invoice_posnic_number').html('<?php echo $this->lang->line('purchase_invoice')." ".$this->lang->line('no') ?> <span class="text-muted "> #'+data[0][0]['invoice_id']+'</span>')
                        }else{
                            $('#invoice_posnic_number').hide();
                        }
                        if(data[1]['posnic_date']==1){
                            $('#invoice_posnic_date').show();
                              $('#invoice_posnic_date').html('<?php echo $this->lang->line('purchase_invoice')." ".$this->lang->line('date') ?> : <span class="text-muted">'+data[0][0]['sales_date']+'</span>');
                        }else{
                     ...
                     $('#invoice_posnic_table').append('<thead/><tbody/><tfoot/>');
                        ...
                        var purchase_return_discount=0;                        
                        var purchase_return_frieght=0;                        
                        var purchase_return_round_off_amount=0;                        
                        var purchase_return_sub_total=0;                        
                        var purchase_return_grand=0;                        
                        for(var i=0;i<data[0].length;i++){
                            ...
                            var per=data[i]['discount_per'];
                            var per2=data[i]['discount_per2'];
                            var discount=data[i]['discount_amount'];
                            var discount2=data[i]['discount_amount2'];
                            ...
                            purchase_return_sub_total=parseFloat(purchase_return_sub_total)+parseFloat(total);
                            ...
                            if(data[1]['posnic_item_tax1']==1){
                                $('#posnic_table_body'+i).append('<td id="posnic_table_head_td6">'+tax+'</td>');
                            }
                            if(data[1]['posnic_item_tax2']==1){
                        ...
                        if(data[1]['posnic_purchase_return_subtotal']==1){
                            var num = parseFloat(purchase_return_sub_total);
                            purchase_return_sub_total=num.toFixed(point);
                            ...
                            $('#posnic_table_tfoot1').append('<td class="text-right">'+purchase_return_sub_total+' </td>');                            
                        }
    ...
    function invoice_settings(){
        ...
        if($this->session->userdata['purchase_return_per']['invoice_setting']==1){ ?>
            ...
            $.ajax({                                      
                url: "<?php echo base_url('purchase_return/get_invoice_settings') ?>",                      
                data: "", 
                dataType: 'json',               
                success: function(data) {
                    ...
                    data['posnic_purchase_return_subtotal']==1?$('#posnic_purchase_return_subtotal').attr('checked','checked'):$('#posnic_purchase_return_subtotal').removeAttr('checked');
    ...
    function save_invoice_settings(){
        ...
        if($this->session->userdata['purchase_return_per']['invoice_setting']==1){ ?>
              $('#loading').modal('show');
                    ...
                    $.ajax ({
                        url: "<?php echo base_url('index.php/purchase_return/save_invoice_settings')?>",
                        data: inputs,
                        type:'POST',
                        complete: function(response) {
                            if(response['responseText']=='TRUE'){
                                ...
                                purchase_return_invoice($('#purchase_return_guid').val());
                                $('#loading').hide();
                            }else{
                                $.bootstrapGrowl('<?php echo $this->lang->line('you_have_no_permission_to_update')." ".$this->lang->line('invoice_settings'); ?>', { type: "error" });          
                            }
