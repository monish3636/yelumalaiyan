...
?>
    ...
    function purchase_invoice_view(guid){
        <?php
        if($this->session->userdata['sales_bill_per']['view']==1){ ?>     
            ...
            $('#loading').modal('show');
            $.ajax({                                      
                url: "<?php echo base_url() ?>index.php/sales_bill/view_sales_bill/"+guid,                      
                data: "", 
                dataType: 'json',               
                ...
                { 
                    ...
                    $("#parsley_reg #demo_bill_no").val(data[0]['invoice']);
                    $("#parsley_reg #delivery_date").val(data[0]['dn_date']);
                    $("#parsley_reg #note").val(data[0]['note']);
                    $("#parsley_reg #bill_date").val(data[0]['date']);
                    $("#parsley_reg #remark").val(data[0]['remark']);
                    $("#parsley_reg #demo_order_number").select2('data', {id:'',text: data[0]['dn_code']});
   ...
   function sales_bill_invoice(guid){
        ...
        if($this->session->userdata['sales_bill_per']['print_invoice']==1){ ?>
            ...
            $('#sales_bill_lists').removeAttr("disabled");
            $('#invoice_div').show();
            $('#sales_bill_guid').val(guid);
            $.ajax({                                      
                url: "<?php echo base_url() ?>index.php/sales_bill/get_invoice_settings_and_sales_bill/"+guid,                      
                data: "", 
                dataType: 'json',               
                success: function(data)        
                { 
                        if(data[1]['posnic_sales_bill_id']==1){
                            $('#invoice_posnic_sales_bill_id').show();
                            $('#invoice_posnic_sales_bill_id').html(data[0][0]['invoice'])
                        }else{
                            $('#invoice_posnic_sales_bill_id').hide();
                        }
                        if(data[1]['posnic_sales_bill_number']==1){
                            $('#invoice_posnic_sales_bill_number').show();
                            $('#invoice_posnic_sales_bill_number').html('<?php echo $this->lang->line('sales_bill')." ".$this->lang->line('id') ?> <span class="text-muted "> #'+data[0][0]['id']+'</span>')
                        }else{
                            $('#invoice_posnic_sales_bill_number').hide();
                        }
                        if(data[1]['posnic_sales_bill_date']==1){
                            $('#invoice_posnic_sales_bill_date').show();
                              $('#invoice_posnic_sales_bill_date').html('<?php echo $this->lang->line('sales_bill')." ".$this->lang->line('date') ?> : <span class="text-muted">'+data[0][0]['date']+'</span>');
                        }else{
                            $('#invoice_posnic_sales_bill_date').hide();
                        }
                        if(data[1]['posnic_order_id']==1){
                            $('#invoice_posnic_id').show();
                            $('#invoice_posnic_id').html('<?php echo $this->lang->line('delivery_note') ?> <span class="text-muted "> '+data[0][0]['dn_code'])
                        }else{
                        ...
                        if(data[1]['posnic_number']==1){
                            ...
                            $('#invoice_posnic_number').html('<?php echo $this->lang->line('delivery_note')." ".$this->lang->line('id') ?> <span class="text-muted "> #'+data[0][0]['dn_id']+'</span>')
                        }else{
                            $('#invoice_posnic_number').hide();
                        }
                        if(data[1]['posnic_date']==1){
                            $('#invoice_posnic_date').show();
                              $('#invoice_posnic_date').html('<?php echo $this->lang->line('delivery_note')." ".$this->lang->line('date') ?> : <span class="text-muted">'+data[0][0]['dn_date']+'</span>');
                        }else{
                     ...
                     $('#invoice_posnic_table').append('<thead/><tbody/><tfoot/>');
                        ...
                        var sales_bill_discount=0;                        
                        var sales_bill_frieght=0;                        
                        var sales_bill_round_off_amount=0;                        
                        var sales_bill_sub_total=0;                        
                        var sales_bill_grand=0;                        
                        for(var i=0;i<data[0].length;i++){
                            ...
                            sales_bill_sub_total=parseFloat(sales_bill_sub_total)+parseFloat(total);
                        ...
                        if(data[1]['posnic_sales_bill_subtotal']==1){
                            var num = parseFloat(sales_bill_sub_total);
                            sales_bill_sub_total=num.toFixed(point);
                            ...
                            $('#posnic_table_tfoot1').append('<td class="text-right">'+sales_bill_sub_total+' </td>');                            
                        }
    ...
    function invoice_enable(){
        $('#bill_date').removeAttr("disabled", "disabled");
    ...
    function invoice_settings(){
        ...
        if($this->session->userdata['sales_bill_per']['invoice_setting']==1){ ?>
            ...
            $.ajax({                                      
                url: "<?php echo base_url('sales_bill/get_invoice_settings') ?>",                      
                data: "", 
                dataType: 'json',               
                success: function(data) {
                    data['posnic_sales_bill_id']==1?$('#posnic_sales_bill_id').attr('checked','checked'):$('#posnic_sales_bill_id').removeAttr('checked'); 
                    data['posnic_sales_bill_number']==1?$('#posnic_sales_bill_number').attr('checked','checked'):$('#posnic_sales_bill_number').removeAttr('checked'); 
                    data['posnic_sales_bill_date']==1?$('#posnic_sales_bill_date').attr('checked','checked'):$('#posnic_sales_bill_date').removeAttr('checked');
                    ...
                    data['posnic_sales_bill_subtotal']==1?$('#posnic_sales_bill_subtotal').attr('checked','checked'):$('#posnic_sales_bill_subtotal').removeAttr('checked');
    ...
    function save_invoice_settings(){
        ...
        if($this->session->userdata['sales_bill_per']['invoice_setting']==1){ ?>
              $('#loading').modal('show');
                    ...
                    $.ajax ({
                        url: "<?php echo base_url('index.php/sales_bill/save_invoice_settings')?>",
                        data: inputs,
                        type:'POST',
                        complete: function(response) {
                            if(response['responseText']=='TRUE'){
                                ...
                                sales_bill_invoice($('#sales_bill_guid').val());
                                $('#loading').hide();
                            }else{
                                $.bootstrapGrowl('<?php echo $this->lang->line('you_have_no_permission_to_update')." ".$this->lang->line('invoice_settings'); ?>', { type: "error" });          
                            }
