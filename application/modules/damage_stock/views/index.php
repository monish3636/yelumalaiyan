...
<script type="text/javascript">
    ...
    function save_new_order(){
         <?php if($this->session->userdata['damage_stock_per']['add']==1){ ?>
                ...
                get_table_data();
                 var inputs = $('#parsley_reg').serialize();
                     $.ajax ({
                            url: "<?php echo base_url('index.php/damage_stock/save')?>",
                            data: inputs,
                            type:'POST',
                            complete: function(response) {
                                if(response['responseText']=='TRUE'){
                                      $.bootstrapGrowl('<?php echo $this->lang->line('damage_stock').' '.$this->lang->line('added');?>', { type: "success" });                                                                                  
                                       $("#dt_table_tools").dataTable().fnDraw();
                                       $("#parsley_reg").trigger('reset');
                                       posnic_damage_stock_lists();
                                       refresh_items_table();
    ...
    function update_order(){
         <?php if($this->session->userdata['damage_stock_per']['edit']==1){ ?>
                   if($('#parsley_reg').valid()){
                       var oTable = $('#selected_item_table').dataTable();
                       if(oTable.fnGetData().length>0){
                        get_table_data();
                        var inputs = $('#parsley_reg').serialize();
                     $.ajax ({
                            url: "<?php echo base_url('index.php/damage_stock/update')?>",
                            data: inputs,
                            type:'POST',
                            complete: function(response) {
                                if(response['responseText']=='TRUE'){
                                      $.bootstrapGrowl('<?php echo $this->lang->line('damage_stock').' '.$this->lang->line('updated');?>', { type: "success" });                                                                                  
                                       $("#dt_table_tools").dataTable().fnDraw();
                                       $("#parsley_reg").trigger('reset');
                                       posnic_damage_stock_lists();
                                       refresh_items_table();
     ...
     $(document).ready( function () {
          ...
          $('#parsley_reg #items').change(function() {
              if(document.getElementById('new_item_row_id_'+$('#parsley_reg #items').select2('data').id) && $('#parsley_reg #diabled_item').val()!=$('#parsley_reg #items').select2('data').id){
                     $.bootstrapGrowl('<?php echo $this->lang->line('this item already added').$this->lang->line('damage_stock');?> ', { type: "warning" });  
                       $('#parsley_reg #items').select2('open');
              }else{
    ...
    return  "<p >"+sup.text+"<img src='<?php echo base_url() ?>/uploads/items/"+sup.image+"' style='float:right;height:59px'></img></p><p style='float:left;width:130px;  margin-left: 10px'> "+sup.value+"</p><p style='float:left;width:130px;  margin-left: 10px'> "+sup.category+"</p> <p style='width:130px;  margin-left: 218px'> "+sup.brand+"</p><p style='width:120px;  margin-left: 380px;margin-top: -28px;'> "+sup.department+"</p>";
          ...
          $('#parsley_reg #items').select2({
              ...
              dropdownCssClass : 'item_select',
                ...
                ajax: {
                     url: '<?php echo base_url() ?>index.php/damage_stock/search_items/',
...
$("#parsley_reg #first_name").val('');
    <?php if($this->session->userdata['damage_stock_per']['add']==1){ ?>
             $.ajax({                                      
                             url: "<?php echo base_url() ?>index.php/damage_stock/order_number/",                      
                             data: "", 
                             dataType: 'json',               
    ...
    $('#add_new_order').show('slow');
      ...
      $('#posnic_add_damage_stock').attr("disabled", "disabled");
      $('#active').attr("disabled", "disabled");
      $('#damage_stock_lists').removeAttr("disabled");
...
function posnic_damage_stock_lists(){
      ...
      $('#posnic_add_damage_stock').removeAttr("disabled");
      $('#damage_stock_lists').attr("disabled",'disabled');
}
function clear_add_damage_stock(){
      $("#parsley_reg").trigger('reset');
      refresh_items_table();
}
function clear_update_damage_stock(){
      $("#parsley_reg").trigger('reset');
      refresh_items_table();
      edit_function($('#damage_stock_guid').val());
}
    ...
    <div class="container">
            <div class="row">
                <div class="col col-lg-7">
                        <a href="javascript:posnic_add_new()" id="posnic_add_damage_stock" class="btn btn-default" ><i class="icon icon-user"></i> <?php echo $this->lang->line('addnew') ?></a>  
                     
                        <a href="javascript:damage_stock_group_approve()" class="btn btn-default" id="deactive"  ><i class="icon icon-play"></i> <?php echo $this->lang->line('approve') ?></a>
                        ...
                        <a href="javascript:posnic_damage_stock_lists()" class="btn btn-default" id="damage_stock_lists"><i class="icon icon-list"></i> <?php echo $this->lang->line('damage_stock') ?></a>
...
<section class="container clearfix main_section">
        <div id="main_content_outer" class="clearfix">
            <div id="main_content">
                    ...
                    echo form_open('damage_stock/damage_stock_manage',$form) ?>
                        <div class="row">
                            <div class="col-sm-12" id="user_list"><br>
                                <div class="panel panel-default">
                                    <div class="panel-heading">
                                            <h4 class="panel-title"><?php echo $this->lang->line('damage_stock') ?></h4>                                                                               
                                    </div>
                                    <table id="dt_table_tools" class="table-striped table-condensed" style="width: 100%"><thead>
                                        <tr>
                                         <th>Id</th>
                                          <th ><?php echo $this->lang->line('select') ?></th>
                                          <th ><?php echo $this->lang->line('damage_stock_id') ?></th>
...
<section id="add_new_order" class="container clearfix main_section">
     <?php   $form =array('id'=>'parsley_reg',
       ...
       echo form_open_multipart('damage_stock/upadate_pos_damage_stock_details/',$form);?>
        
    <div id="main_content" style="padding: 0 14px !important;">
                     
        <input type="hidden" name="stock_id" id="stock_id" >
   
        <input type="hidden" name="damage_stock_guid" id="damage_stock_guid" >
                                ...
                                <div class="col col-sm-6" style="padding-right: 0">
                                          ...
                                          <div class="col col-sm-3" style="padding-top: 50px"  >
                                                   <div class="form_sep " id="save_clear">
                                                       <label for="remark" >&nbsp;</label>	
                                                        <a href="javascript:clear_add_damage_stock()" class="btn btn-default"  ><i class="icon icon-refresh"></i> <?php echo " ".$this->lang->line('clear') ?></a>
                                                  </div>
                                              <div class="form_sep " id="update_clear" style="margin-top:0 !important">
                                                       <label for="remark" >&nbsp;</label>	
                                                        <a href="javascript:clear_update_damage_stock()" class="btn btn-default"  ><i class="icon icon-refresh"></i> <?php echo " ".$this->lang->line('clear') ?></a>
                                                  </div>
                                               </div>
                                     ...
                                     <h4 class="panel-title"><?php echo $this->lang->line('amount') ?></h4>                                                                               
                                                   ...
                                                   <div class="form_sep" style="padding: 0 25px">
                                                            <label for="order_number" ><?php echo $this->lang->line('damage_stock_id') ?></label>													
    ...
    <?php echo form_close();?>
     ...
     function posnic_delete(){
            <?php if($this->session->userdata['damage_stock_per']['delete']==1){ ?>
                     var flag=0;
                     var field=document.forms.posnic;
                      ...
                      if (flag<1) {
                          ...
                          $.bootstrapGrowl('<?php echo $this->lang->line('Select Atleast One')."".$this->lang->line('damage_stock');?>', { type: "warning" });
                            ...
                            bootbox.confirm("<?php echo $this->lang->line('Are you Sure To Delete')."".$this->lang->line('damage_stock') ?>", function(result) {
             if(result){
                        ...
                        for (i = 0; i < posnic.length; i++){
                          ...
                          if(posnic[i].checked==true){ 
                              ...
                              $.ajax({
                                url: '<?php echo base_url() ?>/index.php/damage_stock/delete',
                                type: "POST",
                                data: {
                                    guid:posnic[i].value
                                  ...
                                  complete: function(response) {
                                    if(response['responseText']=='TRUE'){
                                           $.bootstrapGrowl($('#order__number_'+guid).val()+ ' <?php echo $this->lang->line('damage_stock') ?>  <?php echo $this->lang->line('deleted');?>', { type: "error" });
                                        $("#dt_table_tools").dataTable().fnDraw();
                                         ...
                                         $.bootstrapGrowl('<?php echo $this->lang->line('You Have NO Permission')." ".$this->lang->line('to')." ".$this->lang->line('approve')." ".$this->lang->line('damage_stock');?>', { type: "error" });                       
                                    }
                                    }
                            });
                      ...
                      <?php }else{?>
                                   $.bootstrapGrowl('<?php echo $this->lang->line('You Have NO Permission To Delete')." ".$this->lang->line('damage_stock');?>', { type: "error" });                       
                           <?php }
                        ?>
                      }
    ...
    function damage_stock_group_approve(){
         <?php if($this->session->userdata['damage_stock_per']['approve']==1){ ?>
                     var flag=0;
                     var field=document.forms.posnic;
                      for (i = 0; i < field.length; i++){
                          if(field[i].checked==true){
                      ...
                      for (i = 0; i < posnic.length-1; i++){
                          ...
                          if(posnic[i].checked==true){                             
                                 $.ajax({
                                    url: '<?php echo base_url() ?>/index.php/damage_stock/damage_stock_approve',
                                    type: "POST",
                                    data: {
                                        guid: posnic[i].value
                                     ...
                                     complete: function(response) {
                                        if(response['responseText']=='TRUE'){
                                               $.bootstrapGrowl($('#order__number_'+guid).val()+ ' <?php echo $this->lang->line('damage_stock') ?>  <?php echo $this->lang->line('approved');?>', { type: "success" });
                                            $("#dt_table_tools").dataTable().fnDraw();
                                        }else if(response['responseText']=='Approved'){
