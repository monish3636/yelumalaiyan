...
<script type="text/javascript">
    ...
    function save_new_order(){
         <?php if($this->session->userdata['stock_transfer_per']['add']==1){ ?>
                ...
                var inputs = $('#parsley_reg').serialize();
                      $.ajax ({
                            url: "<?php echo base_url('index.php/stock_transfer/save')?>",
                            data: inputs,
                            type:'POST',
                            complete: function(response) {
                                if(response['responseText']=='TRUE'){
                                      $.bootstrapGrowl('<?php echo $this->lang->line('stock_transfer').' '.$this->lang->line('added');?>', { type: "success" });                                                                                  
                                       $("#dt_table_tools").dataTable().fnDraw();
                                       $("#parsley_reg").trigger('reset');
                                       posnic_stock_transfer_lists();
                                       refresh_items_table();
    ...
    function update_order(){
         <?php if($this->session->userdata['stock_transfer_per']['edit']==1){ ?>
                ...
                var inputs = $('#parsley_reg').serialize();
                      $.ajax ({
                            url: "<?php echo base_url('index.php/stock_transfer/update')?>",
                            data: inputs,
                            type:'POST',
                            complete: function(response) {
                                if(response['responseText']=='TRUE'){
                                      $.bootstrapGrowl('<?php echo $this->lang->line('stock_transfer').' '.$this->lang->line('updated');?>', { type: "success" });                                                                                  
                                       $("#dt_table_tools").dataTable().fnDraw();
                                       $("#parsley_reg").trigger('reset');
                                       posnic_stock_transfer_lists();
                                       refresh_items_table();
     ...
     $(document).ready( function () {
       ...
       $('#parsley_reg #select_branch').change(function() {
            refresh_items_table();
    ...
    return  "<p >"+sup.text+"    <br>"+sup.name+"   "+sup.phone+"</p> ";
          ...
          $('#parsley_reg #select_branch').select2({
              dropdownCssClass : 'customers_select',
                ...
                ajax: {
                     url: '<?php echo base_url() ?>index.php/stock_transfer/search_branch',
          ...
          $('#parsley_reg #items').change(function() {
              if(document.getElementById('new_item_row_id_'+$('#parsley_reg #items').select2('data').id) && $('#parsley_reg #diabled_item').val()!=$('#parsley_reg #items').select2('data').id){
                     $.bootstrapGrowl('<?php echo $this->lang->line('this item already added').$this->lang->line('stock_transfer');?> ', { type: "warning" });  
                       $('#parsley_reg #items').select2('open');
              }else{
    ...
    return  "<p >"+sup.text+"<img src='<?php echo base_url() ?>/uploads/items/"+sup.image+"' style='float:right;height:59px'></img></p><p style='float:left;width:130px;  margin-left: 10px'> "+sup.value+"</p><p style='float:left;width:130px;  margin-left: 10px'> "+sup.category+"</p> <p style='width:130px;  margin-left: 218px'> "+sup.brand+"</p><p style='width:120px;  margin-left: 380px;margin-top: -28px;'> "+sup.department+"</p>";
          ...
          $('#parsley_reg #items').select2({
              ...
              dropdownCssClass : 'item_select',
                ...
                ajax: {
                     url: '<?php echo base_url() ?>index.php/stock_transfer/search_items/',
                     data: function(term, page) {
                            return {types: ["exercise"],
                                limit: 2,
                                term: term,
...
$("#parsley_reg #first_name").val('');
    <?php if($this->session->userdata['stock_transfer_per']['add']==1){ ?>
             $.ajax({                                      
                             url: "<?php echo base_url() ?>index.php/stock_transfer/order_number/",                      
                             data: "", 
                             dataType: 'json',               
...
function posnic_stock_transfer_lists(){
      ...
      $('#posnic_add_stock_transfer').removeAttr("disabled");
      $('#stock_transfer_lists').attr("disabled",'disabled');
}
function clear_add_stock_transfer(){
      $("#parsley_reg").trigger('reset');
      refresh_items_table();
}
function clear_update_stock_transfer(){
      $("#parsley_reg").trigger('reset');
      refresh_items_table();
      edit_function($('#stock_transfer_guid').val());
}
    ...
    <div class="container">
            <div class="row">
                <div class="col col-lg-7">
                        <a href="javascript:posnic_add_new()" id="posnic_add_stock_transfer" class="btn btn-default" ><i class="icon icon-user"></i> <?php echo $this->lang->line('addnew') ?></a>  
                     
                        <a href="javascript:stock_transfer_group_approve()" class="btn btn-default" id="deactive"  ><i class="icon icon-play"></i> <?php echo $this->lang->line('approve') ?></a>
                        ...
                        <a href="javascript:posnic_stock_transfer_lists()" class="btn btn-default" id="stock_transfer_lists"><i class="icon icon-list"></i> <?php echo $this->lang->line('stock_transfer') ?></a>
...
<section class="container clearfix main_section">
        <div id="main_content_outer" class="clearfix">
            <div id="main_content">
                    ...
                    echo form_open('stock_transfer/stock_transfer_manage',$form) ?>
                        <div class="row">
                            <div class="col-sm-12" id="user_list"><br>
                                <div class="panel panel-default">
                                    <div class="panel-heading">
                                            <h4 class="panel-title"><?php echo $this->lang->line('stock_transfer') ?></h4>                                                                               
                                    </div>
...
<section id="add_new_order" class="container clearfix main_section">
     <?php   $form =array('id'=>'parsley_reg',
       ...
       echo form_open_multipart('stock_transfer/upadate_pos_stock_transfer_details/',$form);?>
        
    <div id="main_content" style="padding: 0 14px !important;">
                     
        <input type="hidden" name="stock_id" id="stock_id" >
   
        <input type="hidden" name="stock_transfer_guid" id="stock_transfer_guid" >
                        
                         
                      <div class="row">
                          <div class="panel panel-default">
                              <div class="panel-heading" >
                                     <h4 class="panel-title"><?php echo $this->lang->line('stock_transfer')." ".$this->lang->line('details') ?></h4>                                                                               
                               </div>
                                          ...
                                          <div class="col col-sm-3" style="padding-top: 50px"  >
                                                   <div class="form_sep " id="save_clear">
                                                       <label for="remark" >&nbsp;</label>	
                                                        <a href="javascript:clear_add_stock_transfer()" class="btn btn-default"  ><i class="icon icon-refresh"></i> <?php echo " ".$this->lang->line('clear') ?></a>
                                                  </div>
                                              <div class="form_sep " id="update_clear" style="margin-top:0 !important">
                                                       <label for="remark" >&nbsp;</label>	
                                                        <a href="javascript:clear_update_stock_transfer()" class="btn btn-default"  ><i class="icon icon-refresh"></i> <?php echo " ".$this->lang->line('clear') ?></a>
                                                  </div>
                                               </div>
                     ...
                     var field=document.forms.posnic;
                      ...
                      if (flag<1) {
                          ...
                          $.bootstrapGrowl('<?php echo $this->lang->line('Select Atleast One')."".$this->lang->line('stock_transfer');?>', { type: "warning" });
                            ...
                            bootbox.confirm("<?php echo $this->lang->line('Are you Sure To Delete')."".$this->lang->line('stock_transfer') ?>", function(result) {
             if(result){
    ...
    function stock_transfer_group_approve(){
