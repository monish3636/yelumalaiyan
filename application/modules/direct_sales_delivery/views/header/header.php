...
function direct_sales_delivery_approve(guid){
        ...
        if($this->session->userdata['direct_sales_delivery_per']['edit']==1){ ?>
            ...
            $.ajax({                                      
                ...
                { 
                    ...
                    $("#parsley_reg #direct_sales_delivery_guid").val(guid);
    ...
    function direct_sales_delivery_note_view(guid){
        <?php
        if($this->session->userdata['direct_sales_delivery_per']['view']==1){ ?>
            $('#deleted').remove();
            $('#parent_items').append('<div id="deleted"></div>');
            $('#newly_added').remove();
            $('#parent_items').append('<div id="newly_added"></div>');
            ...
            $('#loading').modal('show');
            $.ajax({                                      
                url: "<?php echo base_url() ?>index.php/direct_sales_delivery/view_direct_sales_delivery/"+guid,                      
                data: "", 
                dataType: 'json',               
                success: function(data)        
                { 
    ...
    function direct_sales_delivery_note_invoice(guid){
        <?php
        if($this->session->userdata['direct_sales_delivery_per']['print_invoice']==1){ ?>
            $('#loading').modal('show');
            $("#user_list").hide();
            $('#add_new_order').hide();
            ...
            $('#direct_sales_delivery_lists').removeAttr("disabled");
            $('#invoice_div').show();
            $('#direct_sales_delivery_guid').val(guid);
            $.ajax({                                      
                url: "<?php echo base_url() ?>index.php/direct_sales_delivery/get_invoice_settings_and_direct_sales_delivery/"+guid,                      
                data: "", 
                dataType: 'json',               
                success: function(data)        
                { 
                     ...
                     $('#invoice_posnic_table').append('<thead/><tbody/><tfoot/>');
                        ...
                        var direct_sales_delivery_discount=0;                        
                        var direct_sales_delivery_frieght=0;                        
                        var direct_sales_delivery_round_off_amount=0;                        
                        var direct_sales_delivery_sub_total=0;                        
                        var direct_sales_delivery_grand=0;                        
                        for(var i=0;i<data[0].length;i++){
                            ...
                            direct_sales_delivery_sub_total=parseFloat(direct_sales_delivery_sub_total)+parseFloat(total);
                        ...
                        if(data[1]['posnic_direct_sales_delivery_subtotal']==1){
                            var num = parseFloat(direct_sales_delivery_sub_total);
                            direct_sales_delivery_sub_total=num.toFixed(point);
                            ...
                            $('#posnic_table_tfoot1').append('<td class="text-right">'+direct_sales_delivery_sub_total+' </td>');                            
                        }
                        ...
                        if(data[1]['posnic_customer_discount']==1){
                            ...
                            value=num.toFixed(point);
                            if(isNaN(parseFloat(value))) value=0;
                            $('#invoice_posnic_table tfoot').append('<tr id="posnic_table_tfoot51" class="item-row"><td colspan="'+parseInt($('#invoice_posnic_table thead tr td').length-2)+'"></td></tr>');
                            $('#posnic_table_tfoot51').append('<td ><?php echo $this->lang->line('customer')." ".$this->lang->line('discount') ?> </td>');
                            $('#posnic_table_tfoot51').append('<td class="text-right">'+value+' </td>');                            
                        }
    ...
    function invoice_settings(){
        <?php
        if($this->session->userdata['direct_sales_delivery_per']['invoice_setting']==1){ ?>
            $('#loading').modal('show');
            $('#invoice_div').hide();
            $('#invoice_settings').show('slow');
            $.ajax({                                      
                url: "<?php echo base_url('direct_sales_delivery/get_invoice_settings') ?>",                      
                data: "", 
                dataType: 'json',               
                success: function(data) {
                    ...
                    data['posnic_direct_sales_delivery_subtotal']==1?$('#posnic_direct_sales_delivery_subtotal').attr('checked','checked'):$('#posnic_direct_sales_delivery_subtotal').removeAttr('checked');
    ...
    function save_invoice_settings(){
        ...
        if($this->session->userdata['direct_sales_delivery_per']['invoice_setting']==1){ ?>
              $('#loading').modal('show');
                    ...
                    $.ajax ({
                        url: "<?php echo base_url('index.php/direct_sales_delivery/save_invoice_settings')?>",
                        data: inputs,
                        type:'POST',
                        complete: function(response) {
                            if(response['responseText']=='TRUE'){
                                ...
                                direct_sales_delivery_note_invoice($('#direct_sales_delivery_guid').val());
                                $('#loading').hide();
                            }else{
                                $.bootstrapGrowl('<?php echo $this->lang->line('you_have_no_permission_to_update')." ".$this->lang->line('invoice_settings'); ?>', { type: "error" });          
                            }
