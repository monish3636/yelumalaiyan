...
function save(){      
     if($this->session->userdata['keyboard_sales_per']['add']==1){
        ...
        $this->form_validation->set_rules('new_item_stock_id[]', $this->lang->line('new_item_stock_id'), 'required');                      
            ...
            if ( $this->form_validation->run() !== false ) {    
              ...
              $guid=   $this->posnic->posnic_add_record($value,'keyboard_sales');
                ...
                for($i=0;$i<count($item);$i++){
                    ...
                    $this->load->model('sales');
                    $this->sales->add_keyboard_sales($guid,$item[$i],$quty[$i],$stock[$i],$item_discount[$i],$i);
    ...
    function update(){
            if(isset($_POST['keyboard_sales_guid'])){
      if($this->session->userdata['keyboard_sales_per']['edit']==1){
        $this->form_validation->set_rules('customers_guid',$this->lang->line('customers_guid'), 'required');
        $this->form_validation->set_rules('order_date', $this->lang->line('order_date'), 'required');                      
        $this->form_validation->set_rules('grand_total', $this->lang->line('grand_total'), 'numeric');                      
        $this->form_validation->set_rules('total_amount', $this->lang->line('total_amount'), 'numeric'); 
        ...
        $this->form_validation->set_rules('items_stock[]', $this->lang->line('items_stock')); 
            ...
            if ( $this->form_validation->run() !== false ) {    
              ...
              $guid=  $this->input->post('keyboard_sales_guid');
              $update_where=array('guid'=>$guid);
             $this->posnic->posnic_update_record($value,$update_where,'keyboard_sales');
          
                $sq=  $this->input->post('sq_items');
                $quty=  $this->input->post('items_quty');
                for($i=0;$i<count($sq);$i++){
                    $this->load->model('sales');
                    $this->sales->update_keyboard_sales($sq[$i],$quty[$i]);
    ...
    function save_sales_bill(){
      ...
      if($this->session->userdata['keyboard_sales_per']['edit']==1){
        ...
        $this->form_validation->set_rules('keyboard_sales_guid', $this->lang->line('keyboard_sales_guid'), 'required'); 
        $this->form_validation->set_rules('keyboard_sales_bill_number', $this->lang->line('keyboard_sales_bill_number'), 'required'); 
        
        
            if ( $this->form_validation->run() !== false ) {    
                $customer=  $this->input->post('customers_guid');
                $keyboard_sales_guid=  $this->input->post('keyboard_sales_guid');
             
                $date= strtotime($this->input->post('sales_bill_date'));
                $bill_no= $this->input->post('keyboard_sales_bill_number');
                $remark=  $this->input->post('remark');
                $note=  $this->input->post('note');
               
                $value=array('customer_id'=>$customer,'invoice'=>$bill_no,'date'=>$date,'keyboard_sales_id'=>$keyboard_sales_guid,'remark'=>$remark,'note'=>$note);
               $invoice= $this->posnic->posnic_add_record($value,'sales_bill');
                $this->load->model('sales');
                $this->sales->bill_status($keyboard_sales_guid);
            
                 $this->posnic->posnic_master_increment_max('sales_bill')  ;
                 $this->sales->payable_amount($customer,$keyboard_sales_guid,$invoice)   ;
                echo 'TRUE';
   ...
   if($this->session->userdata['brands_per']['delete']==1){ // check permission of current user for delete purchase  order
            if($this->input->post('guid')){ 
                ...
                $status=$this->sales->check_approve($guid);// check if the purchase order was already apparoved or what
                    if($status!=FALSE){
                        $this->posnic->posnic_delete($guid,'keyboard_sales'); // delete the purchase order
                        echo 'TRUE';
                    }else{
                        echo 'Approved';
                    }
